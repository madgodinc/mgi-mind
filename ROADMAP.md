...
Documentation and recipes on top of the 2.0 surfaces, with no server change.
...
- **Pinned memory blocks (Letta-style core memory).** `mgimind block
  ...
  context render (always-true context rather than ranked retrieval) with 4 KB /
  32-block caps.
...
- **✅ Shipped: v2.4.0 (2026-07-05), multi-tenant confinement gate closed
  ...
  - Scoped-token `/memory/ingest` confines fact/procedure candidates
    (skip-with-counter), since they would otherwise land in the GLOBAL stores,
  ...
  - `/memory/by-agent` confined to the token's allowlist (server-injected
    library filter) instead of a blanket 403, which confines the caller
    instead of locking it out;
    falsifiable contract test.
  - Ciphertext-only newtype on the local backup write path, so the sole writer
  ...
  - Tamper-evidence **audit-log hash-chain** + `mgimind audit verify` (BLAKE3
    ...
    shipped).
- **Remote brain for MCP clients (`mgimind mcp --remote`).** `serve-http`
  already exposes the allowlisted tool surface over HTTP with per-agent
  tokens, but the MCP server itself only ever opens a local store, so "run
...
- **Bench-gated backlog (dedicated bench night, not a calendar minor).**
  ...
  window / AST-aware chunking (audit #20) is retrieval-path, **mandatory
...
## v3.0 horizon: candidates, not promises
...
- Markdown as a live source of truth. Qdrant is canonical; md is export +
  reconcile.
- OAuth / DCR multi-user remote is v3 Candidate C, **not** a v2.x item.
