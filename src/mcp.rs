...
/// tool text. Shared by the MCP stdio loop and the loopback HTTP API in
/// http_api.rs. stdio-independent: takes config as a parameter and returns a
/// value, so it is safe to call from an axum handler.
