...
That number does not change in v1.6. The v1.4 changes are about *what memory means*, not *how it's retrieved*. Retrieval improves later (issue #16). The model changed.

## The problem that drove v1.4

Default RAG memory accumulates contradictions like garbage. You log "I use Rust" today. Tomorrow you switch to Go and log "I use Go". The old fact stays in the store. Search returns both, ranked by cosine similarity. The model has no signal that the new fact supersedes the old one.

The original [audit #13](https://github.com/madgodinc/mgi-mind/pull/2) flagged this. A contributor (@spikefcz) proposed a fix: a `single_valued_predicates` list in config that auto-supersedes single-valued facts. Narrow, correct, would have shipped in an afternoon.

I closed the PR.

The proposal was right, but the same machinery needs to handle three other problems and a flat list does not generalize:

1. **Some predicates are multi-valued.** "Mad uses Rust" + "Mad uses TypeScript" are both true. A single-valued treatment conflicts them.
...
3. **Some single-valued facts deserve to stay live.** "Email is `mad@example.com`" might be a typo overwriting a real address; you do not want to invalidate the real one on the first write.
...
1. **Future evidence can reverse the duel.** If the loser was right and you delete it, you cannot recover.
...
3. **STALE benchmark behavioural metric.** `state_resolution` and `premise_resistance` need the loser still readable; deletion makes them read zero with no signal that anything was dropped.
...
The right weights depend on what the memory is *for*. If you're using mgi-mind as a single-user chat-assistant memory (the default), `confirmations` is weak: one person saying the same thing twice is almost no evidence. `dependants` (how many other facts structurally depend on this one) is what carries the weight.

If you're using it as a CI loop's memory (test outcomes flow in via `mind_outcome`), `external` is the strongest signal. A passing test is harder to lie about than a conversational repetition.

If you're using it as a multi-tenant store (multiple distinct agents writing), `confirmations` carries weight again, for a different reason: independent agents reaching the same conclusion is real evidence, not the same agent repeating itself.
...
The doubt window (v1.4 Phase 3) introduces an anti-ossification mechanism: a fact that gets retrieved without confirming context drift starts a counter; after N drifted retrievals, the fact enters the doubt window and its confidence is halved in ranking until a non-drifted retrieval resets the counter.

The blind spot: facts that are never retrieved never enter the doubt window. A background pass has to re-test top-N entrenched facts. v1.4 scaffolded this (`spawn_background_retest_loop`) but the body was a placeholder, `n_processed_this_tick = 0`.
...
3. Compares to cached score. Three transitions possible via `decide_retest_transition`:
   - **PromoteToDoubt** — both `delta < -0.2` AND `new < 0.3`. Two reasons must agree before state changes. Avoids flipping mid-band facts on noise.
   - **RecoverFromDoubt** — already in doubt AND `delta > +0.2`.
   - **NoChange** — write back new score, continue.
4. Writes audit entry on every transition (`AuditOp::RetestPromote` / `::RetestRecover`; `NoChange` is not logged, to keep the file from ballooning).

**`RetestTransition` enum is exhaustive: there is no `Remove` variant.** Mechanism 1 invariant lives in the type system, not convention.
...
- **`cited_by` self-citation guard depends on cached confidence_score.** Facts that haven't been through the retest pass yet read as confidence 0.5 (default), so they default-block. v1.7 may add a separate "pre-rest" baseline if this surfaces as a problem.
...
If you want to follow along: [Discussions](https://github.com/madgodinc/mgi-mind/discussions) is now enabled. PR templates are in `.github/`. CONTRIBUTING.md describes the branch model.
