...
in MiniLM space at our chunking. It is conservative, because we want to
...
"confirmed" subset looks like vs "unconfirmed", which determines
...
**Output:** `calibration_report.md` (local only, since it references real
...
flash-tier judge. Materially more on a frontier judge, and not
...
Not gated on the Phase 4.3 number; the preprint goes out regardless
...
- Phase 2 spot-check selections of real facts ("known
  high-entrenchment 10 examples"), inspected locally to
...
LongMemEval-S sessions and questions, a *public corpus*. Mad's real
...
the design, which is settled. On the gates: are the right
