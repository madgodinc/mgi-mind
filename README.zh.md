...
- **交叉编码器重排。** 融合后的候选由交叉编码器 (bge-reranker-base) 把查询和文本一起读、重新打分，
  ...
  [语言与重排器](#语言与重排器)）。
- **常驻守护进程。** 长期运行的进程把模型常驻内存，通过 Unix 套接字服务助手 - 一次查询只要毫秒级，
  而不必每次重新加载模型。
...
需要 Rust 工具链 (`rustup`) 和 Node 或 Bun（用于 MCP 服务器）。下面是 Linux；macOS 和 Windows 构建方式相同。
...
# 3. 启动向量数据库和常驻守护进程
target/release/mgimind serve           # 内置 Qdrant（仅本地回环）
target/release/mgimind daemon &         # 让模型保持常驻

# 4. 试一下
target/release/mgimind create work
target/release/mgimind add work "部署服务器 10.0.0.5:8080，SSH 用 deploy@"
target/release/mgimind search "怎么连到部署服务器"
```
...
> **把守护进程作为服务运行。** 第 3 步用 `&` 在前台启动。日常使用请让它一直运行，助手就不会遇到冷启动。
> 一个最小的 systemd 用户单元：
>
> ```ini
...
> ```
>
> `systemctl --user enable --now mgimind-daemon`。没有守护进程也能用，只是 MCP 服务器每次调用都会启动
> CLI 并重新加载模型。
...
MCP 服务器在守护进程运行时连它，否则回退到 CLI，两种方式都能用。给助手看一次
...
```
  你的 AI 助手
        |  MCP (stdio)
        v
  mcp-server (Node)  --- Unix 套接字 --->  mgimind daemon (Rust，模型常驻)
        |  回退：启动 CLI                        |
        v                                        v
  mgimind (Rust CLI) ----------------------> Qdrant（本地，仅回环）
                                                 |
...
| 命令 | 作用 |
|---|---|
| `mgimind serve` / `mgimind stop` | 启动 / 停止内置 Qdrant。 |
| `mgimind daemon` | 常驻守护进程（加载一次模型，服务 MCP 客户端）。 |
...
- **搜索慢。** 那是 CPU 上的重排。调小 `rerank_top_k`，或设 `rerank_enabled = false`。并让守护进程运行，保持模型常驻。
- **每次调用都冷启动。** 守护进程没运行。启动 `mgimind daemon`（见快速开始里的服务说明）。
- **`serve` 报维度不匹配。** 集合向量与 `vector_size` 不符，通常发生在换模型后。用 `mgimind migrate` 重新嵌入。
...
- Vault 使用 AES-256-GCM，密钥由 Argon2id 派生（参数固定，库升级不会把你锁在外面）。仅限终端：主密码和
  解密后的密钥从不经 MCP 通道，守护进程套接字仅属主可用 (0600)。
- 文件写入是原子的并 fsync 目录 - 崩溃后留下旧文件或新文件，而非损坏文件。

## 状态与审计

当前版本：**0.7.x**。项目完成了一次完整代码审计（27 个问题）：**21 个完全修复，6 个部分修复**
