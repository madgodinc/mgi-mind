...
`doctor --fix` 把模型下载到 `~/mgimind/`：嵌入模型
（multilingual-e5-base，量化 ONNX，约 270 MB）以及重排器
（bge-reranker-base，量化 ONNX，约 280 MB）。ONNX Runtime 和 Qdrant 程序
放在 `mgimind` 程序自身旁边，所以 `doctor` 报告缺少库时，指的是安装目录，
不是 `~/mgimind/`。
