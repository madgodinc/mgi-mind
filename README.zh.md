...
一条命令。安装脚本会下载与你系统匹配的程序、放到 PATH 上，并自动运行
`init` + `doctor --fix`（这会拉取 Qdrant、ONNX Runtime 和模型）。
...
结束时它会打印接入 Claude Code 的现成命令：

```bash
claude mcp add mgimind -- /home/you/.local/bin/mgimind mcp
```

就这样。`mgimind mcp` 本身就是 MCP 服务器；它运行整个会话、模型常驻，并在首次使用时拉起
内置的 Qdrant - 没有要单独照看的服务。给助手看一次
...
- `INSTALL_DIR=/opt/mgimind curl ... | sh` - 装到 `~/.local/bin` 以外的目录。
- `MGIMIND_TAG=v0.8.0 curl ... | sh` - 锁定具体版本，不用 `latest`。
- `SKIP_DOCTOR=1 curl ... | sh` - 只放程序；`init` + `doctor --fix` 之后自己跑。

### 手动安装（不用脚本）

如果你不想把 sh 脚本灌进 shell - 从 [Releases](https://github.com/madgodinc/mgi-mind/releases/latest)
下载对应系统的压缩包，把 `mgimind` 放到 PATH 上，然后：
