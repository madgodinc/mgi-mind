...
1. **Mechanism 1 invariant: never hard-delete.** A fact that loses
   ...
   RecoverFromDoubt}`, and there is intentionally no `Remove` variant.
...
2. **§10 q5 guarantees on the background loop.** Any change to
   ...
   - (a) never concurrent with MCP tool call, via `is_mcp_busy()`
     checks at outer wake AND between facts in the walk.
   - (b) hard per-tick cap, via `select_retest_candidates(_, cap)`
     enforces `cap` with an assert.
   - (c) load-aware cadence, where `loadavg_multiplier()` reads
...
1. **Tool surface in `src/mcp.rs`**: add the JSON schema in the
   ...
   `tools_list_returns_v1_5_surface`), which pin the surface size.
...
- PRs land via "Merge with merge commit" rather than squash, because the per-step
...
Claude Code" if you want, as long as the commit history stays clean.

## CHANGELOG

Every user-visible change gets a CHANGELOG.md entry under the
current version heading. A bugfix that only affects internal tests is
optional. Anything that affects MCP surface, CLI flags, formula
behaviour, or build/install is required.
